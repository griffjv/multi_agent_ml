...
def update_double_Q(Q1, Q2, agent, previous_state, previous_action,
                    reward, alpha, gamma, current_state=None):
    # Double Q update formula:
    action_qs = Q1[current_state][:]  # Q values of all actions available
    max_val = max(action_qs)
    max_indices = np.where(action_qs == max_val)
    opt_act = random.choice(max_indices[0])
    Q1[previous_state][previous_action] += alpha * (
            reward + gamma * Q2[current_state][opt_act] - Q1[previous_state][previous_action])
    return Q1
...
def battle_q_learning(environment, num_episodes,  alpha, preload_Q = {}, gamma=0.99,
    ...
    if len(Q_val_A) < 1:
        Q_val_A = defaultdict(lambda: np.zeros(nA))  # default dict doesn't raise key error if one doesn't exist, instead it
        Q_val_B = defaultdict(lambda: np.zeros(nA))  # default dict doesn't raise key error if one doesn't exist, instead it
    # sets default value
    epsilon = eps_start
    ...
    while i_ep <= num_episodes:
        ...
        if i_ep % 50 == 0:
            r = average_red_reward(environment, q_policy, random_policy, Q_val_A, max_episodes=10, max_steps=100000)
            reward_hist.append(r)
            print("episode number: ", i_ep)
            print("size of Q_val: ", len(Q_val_A))
            print("Enemy observations: ", enemy_obs)
        i_ep += 1
        epsilon = max(1 - i_ep / num_episodes, eps_min)
    return Q_val_A, reward_hist, total_learning_steps
...
if full_run == 1:
    ...
    with open('Q.pkl', 'rb') as f:
        Q = dill.load(f)
        if continue_run == 1:
            Q, rewards_history, total_steps = battle_q_learning(env, iterats, 0.1, preload_Q=Q)
            print("length of Q: ", len(Q))
            print("total steps: ", total_steps)
            with open('Q.pkl', 'wb') as d:
                dill.dump(Q, d)
# evaluate q policy
